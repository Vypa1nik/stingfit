...
import { buildProgressSnapshot, formatKg, formatVolume, summarizeSession } from '@/features/fitness/fitnessProgress'
    ...
    exercises: [
      {
        ...
        updatedAt: '2026-04-25T11:00:00.000Z',
        sets: [
          { id: 'set-1', sessionExerciseId: 'session-exercise-1', setNumber: 1, weightKg: 100, reps: 8, rir: 1, status: 'completed', completedAt: '2026-04-25T10:10:00.000Z', createdAt: '2026-04-25T10:00:00.000Z', updatedAt: '2026-04-25T10:10:00.000Z' },
          { id: 'set-2', sessionExerciseId: 'session-exercise-1', setNumber: 2, weightKg: 100, reps: 8, rir: 1, status: 'completed', completedAt: '2026-04-25T10:15:00.000Z', createdAt: '2026-04-25T10:00:00.000Z', updatedAt: '2026-04-25T10:15:00.000Z' },
          { id: 'set-3', sessionExerciseId: 'session-exercise-1', setNumber: 3, weightKg: 100, reps: 8, rir: 1, status: 'completed', completedAt: '2026-04-25T10:20:00.000Z', createdAt: '2026-04-25T10:00:00.000Z', updatedAt: '2026-04-25T10:20:00.000Z' },
        ],
      },
      {
        ...
        updatedAt: '2026-04-25T11:00:00.000Z',
        sets: [
          { id: 'set-4', sessionExerciseId: 'session-exercise-2', setNumber: 1, weightKg: 80, reps: 12, rir: 2, status: 'completed', completedAt: '2026-04-25T10:35:00.000Z', createdAt: '2026-04-25T10:00:00.000Z', updatedAt: '2026-04-25T10:35:00.000Z' },
        ],
      },
    ],
  }
...
function replaceExerciseSets(session: FitnessLiveSession, exerciseIndex: number, count: number, weightKg = 100) {
...
describe('fitness progress calculations', () => {
  ...
  test('calculates volume trend from the two newest completed sessions', () => {
    const snapshot = buildProgressSnapshot([makeLowerVolumeSession(), makeSession()])
  ...
  test('builds 12-week training heatmap anchored to the latest completed workout week', () => {
    ...
    const heatmapDays = snapshot.trainingHeatmapWeeks.flatMap((week) => week.days)

    expect(snapshot.trainingHeatmapWeeks).toHaveLength(12)
    expect(snapshot.trainingHeatmapWeeks.every((week) => week.days.length === 7)).toBe(true)
    expect(heatmapDays[0]?.date).toBe('2026-02-02')
    expect(heatmapDays.at(-1)?.date).toBe('2026-04-26')
    expect(heatmapDays.find((day) => day.date === '2026-01-25')).toBeUndefined()
    expect(heatmapDays.find((day) => day.date === '2026-02-02')).toMatchObject({ completedWorkoutCount: 1, intensity: 1 })
    expect(heatmapDays.find((day) => day.date === '2026-04-25')).toMatchObject({ completedWorkoutCount: 2, intensity: 2 })
  })
  ...
  test('classifies latest-week muscle volume as target or high using 10-20 working sets', () => {
    ...
    const chest = snapshot.muscleGroupSummaries.find((summary) => summary.muscleGroup === 'chest')
    const back = snapshot.muscleGroupSummaries.find((summary) => summary.muscleGroup === 'back')
  ...
  test('builds 12-week exercise volume leaders from completed working sets', () => {
    const outsideWindow = makeSessionOnDate('outside-window-heavy', '2026-01-25')
    outsideWindow.exercises[0]!.sets = outsideWindow.exercises[0]!.sets.map((set) => ({ ...set, weightKg: 500 }))
  ...
  test('builds chronological 1RM trend points per exercise from completed working sets', () => {
    ...
    expect(snapshot.oneRepMaxSeries[0]).toMatchObject({
      ...
      deltaKg: 63.4,
      points: [
        { sessionId: 'session-0', estimatedOneRepMaxKg: 63.3, weightKg: 50, reps: 8 },
        { sessionId: 'session-1', estimatedOneRepMaxKg: 126.7, weightKg: 100, reps: 8 },
      ],
    })
  })
})
