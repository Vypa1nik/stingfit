...
describe("fitness dead-code cleanup", () => {
	...
	test("error-boundary backup exports fitness JSON instead of legacy encrypted workspace backups", () => {
		...
		const backupExportSource = readText(
			"src/features/fitness/exportFitnessBackup.ts",
		);

		expect(appSource).toContain("@/features/fitness/exportFitnessBackup");
		expect(backupExportSource).toContain(
			"@/features/fitness/fitnessRepository",
		);
		expect(backupExportSource).toContain("@/lib/download");
		expect(appSource).not.toContain("@/lib/backup");
		expect(backupExportSource).not.toContain("@/lib/backup");
	});
