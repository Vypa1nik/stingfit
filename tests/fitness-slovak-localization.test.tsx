...
import {
	STARTER_FITNESS_EXERCISES,
	STARTER_FITNESS_PLANS,
	STARTER_PLAN_STRUCTURES,
} from "@/features/fitness/fitnessSeed";
import type {
	FitnessLiveSession,
...
describe("Slovak StingFit localization", () => {
	...
	test("ships Slovak starter plan and exercise names", () => {
		expect(STARTER_FITNESS_PLANS.map((plan) => plan.name)).toEqual([
			"Tlak / Ťah / Nohy",
			"Vrch / Spodok",
			"Celé telo 3×",
		]);
		...
		expect(STARTER_PLAN_STRUCTURES[0]?.days[0]?.workouts[0]?.name).toBe(
			"Tlakový deň A",
		);
	});
		...
		await act(async () => {
			root.render(
				<LiveTrainingSession
					...
					onUpdateSet={async () => undefined}
					onDuplicateSet={async () => undefined}
					onSkipSet={async () => undefined}
