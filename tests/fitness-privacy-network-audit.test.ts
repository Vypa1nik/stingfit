...
const bannedExchangeMetadataPatterns = [
	/\bdevice(Id|_id)?\b/i,
	/\bip(Address|_address|Metadata|_metadata)?\b/i,
	/telemetry/i,
	/analytics/i,
	/\baccount(Id|_id)?\b/i,
	/cloud/i,
...
describe("StingFit telemetry-free privacy and network audit", () => {
	...
	test("does not use outbound telemetry APIs in app/runtime assets", () => {
		...
		for (const filePath of auditedFiles) {
			...
			for (const pattern of bannedRuntimePatterns) {
				expect(
					source,
					`${relative(process.cwd(), filePath)} contains ${pattern}`,
				).not.toMatch(pattern);
			}
			expect(
				source,
				`${relative(process.cwd(), filePath)} must not call fetch()`,
	...
	test("keeps coach exchange payloads free of device, account, and network metadata", () => {
		const auditedFiles = [
		...
		for (const filePath of auditedFiles) {
			const source = readText(filePath);
			for (const pattern of bannedExchangeMetadataPatterns) {
				expect(source, `${filePath} contains ${pattern}`).not.toMatch(pattern);
			}
		}

		const planPackIo = readText("src/features/coach/planPack/io.ts");
		const recapPackIo = readText("src/features/coach/recapPack/io.ts");
		expect(planPackIo).toContain("coachName: activeProfile.name");
		expect(recapPackIo).toContain("traineeName: activeProfile.name");
		expect(planPackIo).not.toContain("activeProfile.id");
		expect(recapPackIo).not.toContain("activeProfile.id");
	});
	...
	test("documents the completed no-telemetry audit for release handoff", () => {
		...
		expect(report).toContain(
			"No login, no cloud sync, no telemetry, no analytics",
		);
		expect(report).toContain(
			"Service worker fetch is limited to same-origin cache/runtime requests",
		);
		expect(report).toContain(
