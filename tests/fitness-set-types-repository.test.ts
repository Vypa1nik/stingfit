...
describe('fitness set types repository', () => {
  beforeEach(async () => {
  ...
  test('persists warmup set type and preserves it through local export/import', async () => {
    ...
    expect(exported.sessions[0]?.exercises[0]?.sets[0]).toMatchObject({ setType: 'warmup' })
    ...
    expect(restored.exercises[0]?.sets[0]).toMatchObject({ setType: 'warmup' })
  })
})
