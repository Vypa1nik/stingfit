...
describe("FitnessSettingsPage PWA install guidance", () => {
	...
	test("shows install guidance and offline promise without requiring cloud features", async () => {
		...
		expect(container.textContent).toContain("Otvoriť návod na inštaláciu");
		expect(container.textContent).toContain(
			"iOS Safari, Android Chrome a Chrome/Edge",
		);
		const installGuideLink = container.querySelector<HTMLAnchorElement>(
			'a[href="/install.html"]',
		);
		expect(installGuideLink?.textContent).toContain(
