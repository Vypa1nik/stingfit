...
function AppFrame() {
	...
	const setShortcutsCheatsheetOpen = useUiStore(
		(state) => state.setShortcutsCheatsheetOpen,
	);
	...
	const handleBackupExport = useCallback(async () => {
		try {
			const { exportFitnessBackup } = await import(
				"@/features/fitness/exportFitnessBackup"
			);
			...
			pushToast({
				...
				description:
					error instanceof Error
						? error.message
						: "StingFit nedokázal exportovať tréningovú zálohu.",
			});
			throw error;
		}
	}, [pushToast]);
	...
	const actions = useMemo<AppAction[]>(
		() => [
			...
			},
			{
				...
				title: "Otvoriť kalkulačku kotúčov",
		...
		[navigate, setCommandPaletteOpen, setMode, setShortcutsCheatsheetOpen],
	);
			...
			<div className="flex min-h-screen items-center justify-center bg-surface p-6 dark:bg-surface-dark">
				<div className="card-surface max-w-lg p-6">
					<h1 className="text-lg font-semibold text-text-primary dark:text-text-primary-dark">
						StingFit narazil na problém pri štarte
					</h1>
	...
	if (!isReady) {
		return (
			<div className="flex min-h-screen items-center justify-center bg-surface p-6 dark:bg-surface-dark">
				<div className="card-surface max-w-lg p-6">
					<p className="text-sm font-medium uppercase tracking-[0.22em] text-text-muted dark:text-text-muted-dark">
						Načítavam StingFit
					</p>
					<h1 className="mt-2 text-2xl font-semibold text-text-primary dark:text-text-primary-dark">
						Pripravujem tvoj lokálny tréningový priestor...
					</h1>
				</div>
			</div>
		);
