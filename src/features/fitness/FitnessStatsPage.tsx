...
export function FitnessStatsPage() {
	...
	const error = statsQuery.error
		? statsQuery.error instanceof Error
			...
			: sk.fitness.stats.queryError
		: null;

	const snapshot = useMemo(() => buildProgressSnapshot(sessions), [sessions]);
	const heatmapSummary = useMemo(
		() => summarizeHeatmap(snapshot.trainingHeatmapWeeks),
		[snapshot.trainingHeatmapWeeks],
	);
	const thisWeekSummary = useMemo(
		() => summarizeCurrentWeek(sessions),
		[sessions],
	);
	...
	return (
		<div className="space-y-6">
			...
			{error ? (
				<Card
					title={sk.fitness.stats.errorTitle}
					description={sk.fitness.stats.errorDescription}
				>
			...
			{!isLoading && !error && sessions.length > 0 ? (
				<>
					<Card
						title="Ako sa ti darí?"
						description="Najprv jednoduchý týždenný stav. Grafy a detailné odporúčania sú nižšie, keď ich chceš riešiť."
					>
						<div
							data-testid="stats-beginner-summary"
						...
						>
							<div className="rounded-2xl border border-fitness-yellow/30 bg-black p-4 text-fitness-warm">
								...
								<p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
									Tréningy tento týždeň
								</p>
								<p className="mt-2 text-3xl font-black text-fitness-yellow">
									{formatWorkoutCount(thisWeekSummary.workouts)}
								</p>
							</div>
							<div className="rounded-2xl border border-fitness-yellow/30 bg-black p-4 text-fitness-warm">
							...
							<div className="rounded-2xl border border-fitness-yellow/30 bg-black p-4 text-fitness-warm">
								...
								<p className="mt-2 text-sm font-black leading-5 text-fitness-yellow">
									{bestProgressEvent
										? `${bestProgressEvent.exerciseName} · ${formatWeight(bestProgressEvent.estimatedOneRepMaxKg, settings.displayUnit)} e1RM`
										: "Zapíš viac sérií a progres sa ukáže tu."}
								</p>
							</div>
							<div className="rounded-2xl border border-fitness-yellow/30 bg-black p-4 text-fitness-warm">
								...
								<p className="mt-2 text-sm font-black leading-5 text-fitness-yellow">
									{primaryRecoverySignal
										? primaryRecoverySignal.title
										: "Bez varovania"}
								</p>
							</div>
						</div>
					...
					<details className="rounded-3xl border border-fitness-yellow/20 bg-black/55 p-4 text-fitness-warm">
						<summary className="cursor-pointer text-sm font-black uppercase tracking-[0.16em] text-fitness-yellow">
							Podrobné grafy a odporúčania
						</summary>
						<p className="mt-2 text-sm text-fitness-warm/65">
							Otvor, keď chceš riešiť 12-týždňovú konzistentnosť, objem podľa
							svalov, recovery signály alebo 1RM grafy.
						</p>
						<div className="mt-4 space-y-6">
							<section className="grid gap-6 lg:grid-cols-[0.9fr,1.1fr]">
								<Card
									title="Snímka progresu"
									description="Prvé užitočné čísla bez ťažkej analytiky."
								>
								...
								<Card
									title="Nedávne PR udalosti"
									description="Odhadované 1RM PR sa počítajú z dokončených sérií."
								>
									{snapshot.prEvents.length > 0 ? (
										<div className="space-y-3">
											{snapshot.prEvents.map((event) => (
												<div
													key={`${event.exerciseId}-${event.estimatedOneRepMaxKg}`}
													className="flex items-center justify-between rounded-2xl border border-fitness-yellow/20 bg-black px-4 py-3 text-fitness-warm"
												>
													<div>
														<span className="text-sm font-semibold">
															{event.exerciseName} ·{" "}
															{formatWeight(
																event.estimatedOneRepMaxKg,
																settings.displayUnit,
															)}{" "}
															e1RM
														</span>
							...
							<Card
								title="Konzistentnosť 12 týždňov"
								description="Každý štvorček je deň; žltá intenzita ukazuje, koľko tréningov si v daný deň dokončil."
							>
								<div className="grid gap-3 sm:grid-cols-2">
									<div className="rounded-2xl border border-fitness-yellow/20 bg-black px-4 py-4 text-fitness-warm">
										...
										<p className="mt-2 text-2xl font-black text-fitness-yellow">
											{formatTrainingDayCount(heatmapSummary.trainingDays)}
										</p>
									</div>
									<div className="rounded-2xl border border-fitness-yellow/20 bg-black px-4 py-4 text-fitness-warm">
										<p className="text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
										...
										<p className="mt-2 text-2xl font-black text-fitness-yellow">
											{formatWorkoutCount(heatmapSummary.workouts)}
										</p>
									</div>
								</div>
								...
								<div className="mt-5 overflow-x-auto pb-2">
									<div className="inline-flex min-w-max gap-2">
										<div
											className="grid grid-rows-7 gap-1 pr-1 text-[10px] font-black uppercase text-fitness-warm/45"
											aria-hidden="true"
										>
										...
										>
											{snapshot.trainingHeatmapWeeks.map((week) => (
												<div
													...
													aria-label={`Týždeň od ${formatHeatmapDate(week.weekStart)}`}
												>
													{week.days.map((day) => (
														<span
															key={day.date}
								...
								<div className="mt-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-fitness-warm/45">
									...
									{[0, 1, 2, 3, 4].map((intensity) => (
										<span
											key={intensity}
											className={getHeatmapCellClass(
												intensity as FitnessTrainingHeatmapIntensity,
											)}
										/>
									))}
									<span>Viac</span>
							...
							<Card
								title="Objem podľa cviku"
								description="Top cviky podľa pracovného objemu v rovnakom 12-týždňovom okne ako heatmapa."
							>
								{snapshot.exerciseVolumeLeaders.length > 0 ? (
									<div
										data-testid="exercise-volume-leaders"
										className="space-y-3"
									>
										{snapshot.exerciseVolumeLeaders.map((leader) => {
											const maxVolume =
												snapshot.exerciseVolumeLeaders[0]?.totalVolumeKg ?? 1;
											const barWidth = Math.max(
												8,
												Math.round((leader.totalVolumeKg / maxVolume) * 100),
											);
											return (
												<article
													key={leader.exerciseId}
												...
												>
													<div className="flex flex-wrap items-center justify-between gap-3">
														<div>
															...
															<p className="mt-1 text-xs font-bold text-fitness-warm/60">
																{formatVolumeWeight(
																	leader.totalVolumeKg,
																	settings.displayUnit,
																)}{" "}
																·{" "}
																{formatCompletedSetCount(leader.completedSets)}{" "}
																· {formatSessionCount(leader.sessionCount)}
															</p>
														</div>
													...
													<div className="mt-3 h-2 overflow-hidden rounded-full bg-fitness-yellow/10">
														<div
															className="h-full rounded-full bg-fitness-yellow"
									...
									<p className="text-sm text-text-secondary dark:text-text-secondary-dark">
										Zapíš pracovné série a StingFit ukáže, kam išiel tvoj objem.
									</p>
								)}
							</Card>

							<Card
								title="Objem podľa svalovej skupiny"
								description="Pracovné série a objem za posledných 12 týždňov podľa lokálnej taxonómie cvikov."
							>
								{snapshot.muscleGroupSummaries.length > 0 ? (
									<div
										data-testid="muscle-group-volume"
										className="grid gap-3 lg:grid-cols-2"
									>
										{snapshot.muscleGroupSummaries.map((summary) => {
											const maxSets =
												snapshot.muscleGroupSummaries[0]?.completedSets ?? 1;
											const barWidth = Math.max(
												8,
												Math.round((summary.completedSets / maxSets) * 100),
											);
											return (
												<article
												...
												>
													<div className="flex flex-wrap items-start justify-between gap-3">
														<div>
															...
															<p className="mt-1 text-xs font-bold text-fitness-warm/60">
																{formatCompletedSetCount(summary.completedSets)}{" "}
																·{" "}
																{formatVolumeWeight(
																	summary.totalVolumeKg,
																	settings.displayUnit,
																)}{" "}
																· {formatExerciseCount(summary.exerciseCount)}
															</p>
															<p className="mt-1 text-xs font-semibold text-fitness-warm/50">
																Posledný týždeň:{" "}
																{formatCompletedSetCount(
																	summary.latestWeekSets,
																)}{" "}
																·{" "}
																{formatVolumeWeight(
																	summary.latestWeekVolumeKg,
																	settings.displayUnit,
																)}
															</p>
															<p className="mt-1 text-xs font-semibold text-fitness-warm/50">
																Priemer 12 týždňov:{" "}
																{formatWeeklySetAverage(
																	summary.weeklySetAverage,
																)}{" "}
																· Cieľ 10–20 sérií/týždeň
															</p>
														</div>
														<Badge
															className={getMuscleVolumeBadgeClass(
																summary.latestWeekStatus,
															)}
														>
															{formatMuscleVolumeStatus(
																summary.latestWeekStatus,
															)}
														</Badge>
													</div>
													...
													<div className="mt-3 rounded-2xl border border-fitness-yellow/15 bg-fitness-yellow/5 px-3 py-3">
														<p className="text-xs font-black uppercase tracking-[0.16em] text-fitness-yellow/70">
															Akcia: {summary.actionLabel}
														</p>
														<p className="mt-1 text-xs font-semibold leading-5 text-fitness-warm/65">
									...
									<p className="text-sm text-text-secondary dark:text-text-secondary-dark">
										Zapíš pracovné série a StingFit ich rozdelí podľa svalových
										skupín.
									</p>
								)}
							</Card>
							...
							<Card
								title="Regeneračné signály"
								description="Kombinuje objem posledného týždňa, RPE a energiu, aby si vedel kedy pritlačiť a kedy ubrať."
							>
								{snapshot.recoverySignals.length > 0 ? (
											...
											>
												<div className="flex flex-wrap items-start justify-between gap-3">
													<div>
														...
														{signal.muscleGroupLabel ? (
															<p className="mt-1 text-xs font-semibold text-fitness-warm/50">
																Svalová skupina: {signal.muscleGroupLabel}
															</p>
														) : null}
													</div>
													<Badge
														className={getRecoverySignalBadgeClass(
															signal.severity,
														)}
													>
														{formatRecoverySignalSeverity(signal.severity)}
													</Badge>
												</div>
												<p className="mt-3 rounded-2xl border border-fitness-yellow/10 bg-fitness-yellow/5 px-3 py-3 text-xs font-semibold leading-5 text-fitness-warm/65">
													{signal.reason}
												</p>
											</article>
										))}
									...
									<p className="text-sm text-text-secondary dark:text-text-secondary-dark">
										Bez regeneračného varovania. Sleduj techniku, spánok a výkon
										v ďalšom tréningu.
									</p>
								)}
							</Card>
							...
							<Card
								title="Krivka 1RM"
								description="Najlepší odhad 1RM pre rovnaký cvik naprieč dokončenými tréningami."
							>
								{snapshot.oneRepMaxSeries.length > 0 ? (
												...
												<div className="flex flex-wrap items-start justify-between gap-3">
													<div>
														<h2 className="text-sm font-black text-fitness-yellow">
															{series.exerciseName} ·{" "}
															{formatWeight(
																series.latestEstimatedOneRepMaxKg,
																settings.displayUnit,
															)}{" "}
															e1RM
														</h2>
														<p className="mt-1 text-xs font-bold text-fitness-warm/60">
															{formatTrendPointCount(series.points.length)} ·{" "}
															{formatTrendDelta(
																series.deltaKg,
																settings.displayUnit,
															)}
									...
									<p className="text-sm text-text-secondary dark:text-text-secondary-dark">
										Dokonči pracovnú sériu a StingFit zobrazí lokálnu 1RM
										krivku.
									</p>
								)}
							</Card>
							...
							<Card
								title="Odporúčania progresu"
								description="Transparentné dôvody; aplikácia nikdy nemení plán automaticky."
							>
								{snapshot.progressionHints.length > 0 ? (
									...
									<p className="text-sm text-text-secondary dark:text-text-secondary-dark">
										Zapíš celé cieľové série na hornej hranici opakovaní a
										odomkneš odporúčanie.
									</p>
								)}
							</Card>
						</div>
...
function summarizeCurrentWeek(sessions: FitnessLiveSession[]) {
	...
	return {
		...
		volumeKg: summaries.reduce(
			(sum, summary) => sum + summary.totalVolumeKg,
			0,
		),
	};
}
...
function getSessionDateKey(session: FitnessLiveSession) {
	return (
		(session.completedAt ?? session.startedAt ?? session.updatedAt)?.slice(
			0,
			10,
		) ?? null
	);
...
function getHeatmapCellClass(intensity: FitnessTrainingHeatmapIntensity) {
	...
	if (intensity === 0)
		return `${baseClass} border-fitness-yellow/10 bg-fitness-yellow/5`;
	if (intensity === 1)
		return `${baseClass} border-fitness-yellow/25 bg-fitness-yellow/25`;
	if (intensity === 2)
		return `${baseClass} border-fitness-yellow/40 bg-fitness-yellow/45`;
	if (intensity === 3)
		return `${baseClass} border-fitness-yellow/60 bg-fitness-yellow/70`;
	return `${baseClass} border-fitness-yellow bg-fitness-yellow`;
}

function formatHeatmapDate(dateKey: string) {
	return new Date(`${dateKey}T00:00:00.000Z`).toLocaleDateString("sk-SK", {
		day: "numeric",
		month: "short",
	});
}
...
function formatTrendDelta(
	deltaKg: number | null,
	displayUnit: FitnessSettingsRecord["displayUnit"],
) {
	if (deltaKg === null) {
		return "Základ";
	}
