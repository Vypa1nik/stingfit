import {
	...
	resolveMuscleGroup,
} from "@/features/fitness/fitnessMuscleGroups";
...
export function summarizeSession(
	...
	const totalVolumeKg = progressSets.reduce(
		(sum, set) => sum + set.weightKg * set.reps,
		0,
	);
	const totalCorrections = correctedSets.reduce(
		(sum, set) => sum + Math.max(1, set.correctionCount ?? 0),
		0,
	);
...
export function buildProgressSnapshot(
	...
	const completedSetContexts = completedSessions.flatMap(
		getCompletedSetContexts,
	);
	...
	return {
		...
		totalVolumeKg: sessionSummaries.reduce(
		...
		recoverySignals: buildRecoverySignals(
			completedSessions,
			muscleGroupSummaries,
		),
...
function buildTrainingHeatmapWeeks(
	sessions: FitnessLiveSession[],
): FitnessTrainingHeatmapWeek[] {
	...
	const endDate = addUtcDays(
		latestDate,
		6 - getMondayBasedWeekdayIndex(latestDate),
	);
...
function getSessionDateKey(session: FitnessLiveSession) {
	const dateSource =
		session.completedAt ?? session.startedAt ?? session.updatedAt;
...
function buildExerciseVolumeLeaders(
	sessions: FitnessLiveSession[],
): FitnessExerciseVolumeSummary[] {
	const completedDateKeys = sessions
		.filter((session) => session.status === "completed")
	...
	for (const session of sessions) {
		...
		if (
			!sessionDateKey ||
			sessionDateKey < startDateKey ||
			sessionDateKey > endDateKey
		) {
			continue;
		}
		...
		for (const exercise of session.exercises) {
			const completedSets = exercise.sets.filter(
				(set) => set.status === "completed" && isProgressSet(set),
			);
			...
			entry.totalVolumeKg += completedSets.reduce(
				(sum, set) => sum + set.weightKg * set.reps,
				0,
			);
	...
	return Array.from(totalsByExercise.entries())
		...
		.sort(
			(a, b) =>
				...
				a.exerciseName.localeCompare(b.exerciseName, "sk"),
		)
		.slice(0, 5);
}
...
function buildMuscleGroupSummaries(
	...
	const startDateKey = formatDateKey(startDate);
	const endDateKey = formatDateKey(endDate);
	const totalsByMuscleGroup = new Map<
		FitnessMuscleGroup,
		{
			totalVolumeKg: number;
			completedSets: number;
	...
	for (const session of sessions) {
		...
		for (const exercise of session.exercises) {
			...
			const muscleGroup = resolveMuscleGroup({
				...
				muscleGroup: exercise.muscleGroupSnapshot,
			});
			const entry = totalsByMuscleGroup.get(muscleGroup) ?? {
				totalVolumeKg: 0,
				completedSets: 0,
				latestWeekVolumeKg: 0,
				latestWeekSets: 0,
			...
			entry.completedSets += completedSets.length;
			if (
				sessionDateKey >= latestWeekStartKey &&
				sessionDateKey <= endDateKey
			) {
	...
	return Array.from(totalsByMuscleGroup.entries())
		.map(([muscleGroup, entry]) => {
			...
			const action = buildMuscleVolumeAction(
				latestWeekStatus,
				entry.latestWeekSets,
				weeklySetAverage,
			);
			return {
				muscleGroup,
...
function buildMuscleVolumeAction(
	status: FitnessMuscleVolumeStatus,
	latestWeekSets: number,
...
function buildRecoverySignals(
	...
	const highVolumeMuscleGroups = muscleGroupSummaries
		...
		.sort(
			(a, b) =>
				b.latestWeekSets - a.latestWeekSets ||
				b.latestWeekVolumeKg - a.latestWeekVolumeKg ||
				a.label.localeCompare(b.label, "sk"),
		);
	const signals: FitnessRecoverySignal[] = highVolumeMuscleGroups.map(
		(summary) => {
			const highStrainSuffix = strain.hasHighStrain
				? ` Zároveň posledný týždeň hlási ${strain.label}.`
				: "";
			return {
				id: `high-volume-${summary.muscleGroup}`,
				severity: strain.hasHighStrain ? "deload" : "reduce",
				title: strain.hasHighStrain
					? "Regenerácia je pravdepodobne limit"
					: "Objem je nad cieľom",
				recommendation: strain.hasHighStrain
					? "Uber objem a zaraď ľahší tréning"
					: "Uber objem",
...
function buildOneRepMaxSeries(
	sessions: FitnessLiveSession[],
): FitnessOneRepMaxSeries[] {
	const seriesByExercise = new Map<
		string,
		{ exerciseName: string; points: FitnessOneRepMaxPoint[] }
	>();
	const orderedSessions = [...sessions]
		.filter((session) => session.status === "completed")
		.sort((a, b) =>
			String(a.completedAt ?? a.updatedAt ?? "").localeCompare(
				String(b.completedAt ?? b.updatedAt ?? ""),
			),
		);

	for (const session of orderedSessions) {
		const bestPointByExercise = new Map<
			string,
			{ exerciseName: string; point: FitnessOneRepMaxPoint }
		>();
		...
		for (const exercise of session.exercises) {
			for (const set of exercise.sets) {
				...
				const estimatedOneRepMaxKg = estimateOneRepMaxKg(
					set.weightKg,
					set.reps,
				);
				...
				if (
					!currentBest ||
					estimatedOneRepMaxKg > currentBest.point.estimatedOneRepMaxKg
				) {
	...
	return Array.from(seriesByExercise.entries())
		...
		.map(([exerciseId, series]) => {
			...
			const deltaKg = previousPoint
				? Math.round(
						(latestPoint.estimatedOneRepMaxKg -
							previousPoint.estimatedOneRepMaxKg) *
							10,
					) / 10
				: null;
			return {
...
function getCompletedSetContexts(
	session: FitnessLiveSession,
): CompletedSetContext[] {
...
function buildProgressionHint(
	...
	const hitTopRepRange = targetSets.every(
		(set) => set.reps >= exercise.maxReps,
	);
	const hasEnoughRir =
		targetRir === null ||
		targetSets.every((set) => set.rir !== null && set.rir >= targetRir);
