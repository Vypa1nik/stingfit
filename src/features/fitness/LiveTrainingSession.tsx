...
export function LiveTrainingSession({
	...
	const [unplannedExerciseId, setUnplannedExerciseId] = useState(
	...
	const [finishRpe, setFinishRpe] = useState(
		session.sessionRpe === null ? "8" : String(session.sessionRpe),
	);
	const [finishEnergy, setFinishEnergy] = useState(
	...
	const activeExercise =
		visibleSession.exercises.find((exercise) => exercise.status === "active") ??
		visibleSession.exercises.find((exercise) => exercise.status === "pending");
	const activeSupersetExercises = activeExercise?.supersetGroup
		? visibleSession.exercises.filter(
	...
	const lastCompletedSet = activeExercise?.sets
		...
		.at(-1);
	const removableSet = activeExercise
		? (activeExercise.sets.filter((set) => set.status === "planned").at(-1) ??
			activeExercise.sets.at(-1))
		: undefined;
	const selectedUnplannedExerciseId =
		unplannedExerciseId || exerciseOptions[0]?.id || "";
	const exerciseCount = visibleSession.exercises.length;
	const plannedSetCount = visibleSession.exercises.reduce(
		(sum, exercise) => sum + exercise.targetSets,
		0,
	);
	const completedSetCount = visibleSession.exercises
		.flatMap((exercise) => exercise.sets)
		.filter((set) => set.status === "completed").length;
	const totalSets = visibleSession.exercises.flatMap(
	...
	const submitOptimisticSetLog = async (
		setId: string,
				...
				<section className="fitness-hero-panel p-6 lg:p-8">
					<Badge className="fitness-badge">
						{session.exercises.length === 0
							? "Rýchly tréning"
							: "Tréning pripravený na dokončenie"}
					</Badge>
					...
					<p className="mt-3 text-sm text-fitness-warm/70">
						{session.exercises.length === 0
							? "Pridaj prvý cvik z lokálnej knižnice a začni zapisovať série bez plánu."
							: "Všetky cviky sú hotové alebo preskočené."}
					</p>
					<div className="mt-6 grid gap-4 lg:grid-cols-[1fr,auto]">
						...
						<div className="flex flex-col gap-3 lg:items-end">
							<Button
								...
								onClick={() => void onFinish(session.id)}
								disabled={isMutating}
							>
								Dokončiť tréning
							</Button>
		...
		<div className="space-y-6">
			<section className="fitness-hero-panel relative">
				...
				<div className="relative grid gap-6 p-6 lg:grid-cols-[1.35fr,0.95fr] lg:p-8">
					<div className="space-y-5">
						...
						{isActiveSuperset ? (
							<SupersetTransitionCard
								group={activeExercise.supersetGroup ?? ""}
								exercises={activeSupersetExercises.map(
			...
			<details className="rounded-3xl border border-fitness-yellow/20 bg-black/55 p-4 text-fitness-warm">
				<summary className="cursor-pointer text-sm font-black uppercase tracking-[0.16em] text-fitness-yellow">
					Celý tréning a akcie
				</summary>
				<p className="mt-2 text-sm text-fitness-warm/65">
					Otvor, keď chceš preskočiť cvik, pridať sériu, pozrieť poradie alebo
					dokončiť tréning.
				</p>
				<section className="mt-4 grid gap-6 xl:grid-cols-[1.4fr,0.9fr]">
					<Card
						title="Poradie cvikov"
						description={
							showGuidance
								? "Snímka z tvojho osobného plánu. História zostane stabilná aj po neskoršej zmene plánu."
								: undefined
						}
					>
								...
								<article
									...
									className={cn(
										...
										exercise.status === "active"
											? "border-fitness-yellow bg-fitness-yellow/10"
											: "border-fitness-yellow/20 bg-black/70",
									)}
								>
									<div className="flex items-start justify-between gap-4">
										...
										{exercise.status === "active" ? (
											<Badge className="fitness-badge">Aktívny</Badge>
										) : null}
									</div>
								</article>
							))}
					...
					<div className="space-y-6">
						...
						<Card
							...
							description={
								showGuidance
									? "Praktické ovládanie pre reálne zmeny vo fitku."
									: undefined
							}
						>
							<div className="grid gap-3">
								...
								{removableSet && activeExercise.sets.length > 1 ? (
									<Button
										...
										onClick={() => onRemoveSet(removableSet.id)}
										disabled={isMutating}
									>
								...
								<Button
									variant="secondary"
									leadingIcon={<FastForward className="size-4" />}
									onClick={() => onSkipExercise(activeExercise.id)}
									disabled={isMutating}
								>
									Preskočiť cvik
								</Button>
								<Button
									...
									leadingIcon={<Trophy className="size-4" />}
									onClick={() => void onFinish(session.id)}
									disabled={isMutating}
								>
									Dokončiť tréning
						...
						<Card
							title="Pridať neplánovaný cvik"
							description={
								showGuidance
									? "Pre výmenu stroja, pumpu alebo doplnky bez úpravy plánu uprostred tréningu."
									: undefined
							}
						>
							{exerciseOptions.length > 0 ? (
								<div className="space-y-3">
									<label className="block text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
										...
										<select
											...
											onChange={(event) =>
												setUnplannedExerciseId(event.target.value)
											}
										>
									...
									<label className="block text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
										...
										<input
											...
											onInput={(event) =>
												setUnplannedTargetSets(event.currentTarget.value)
											}
										/>
									</label>
									<Button
								...
								<p className="text-sm text-fitness-warm/70">
									Zatiaľ nie sú dostupné žiadne cviky.
								</p>
							)}
						</Card>
						...
						{showGuidance ? (
							<Card
								title="Trénerská pripomienka"
								description="Krátka pripomienka techniky pre aktuálnu sériu."
							>
								<div className="rounded-2xl bg-fitness-yellow px-4 py-4 text-sm font-bold text-black">
									<Zap className="mr-2 inline size-4" /> Najprv zapíš čisté
									opakovania. Váhu zvyšuj až po stabilnom splnení cieľa s
									rezervou.
								</div>
							</Card>
						) : null}
...
function SupersetTransitionCard({
	group,
	exercises,
}: {
	group: string;
	...
	return (
		<div className="rounded-2xl border border-fitness-yellow/30 bg-black/70 p-4">
			...
			<p className="mt-2 text-lg font-black text-white">
				Bez pauzy medzi cvikmi
			</p>
			<p className="mt-1 text-xs font-semibold leading-5 text-fitness-warm/60">
				Striedaj: {exercises.join(" ↔ ")}
			</p>
		</div>
	);
}
		...
		>
			{sets.length > 0 ? (
				<div className="space-y-3">
					{sets.map((set) => (
						<article
							...
							onTouchStart={(event) =>
								startSwipe(set.id, event.changedTouches[0]?.clientX ?? 0)
							}
							onTouchEnd={(event) =>
								finishSwipe(set.id, event.changedTouches[0]?.clientX ?? 0)
							}
						>
							<div className="flex flex-wrap items-center justify-between gap-3">
								<div>
									...
									{formatPerSideWeight(set, displayUnit) ? (
										<p className="mt-1 text-xs font-bold text-fitness-yellow/80">
											{formatPerSideWeight(set, displayUnit)}
										</p>
									) : null}
									{formatCorrectionBadge(set) ? (
										<p className="mt-1 text-xs font-bold text-fitness-orange">
											{formatCorrectionBadge(set)}
										</p>
									) : null}
								</div>
								<div className="flex flex-wrap items-center gap-2">
									<span
										className={cn(
											...
											getFitnessSetTypeBadgeClass(set.setType ?? "working"),
										)}
									>
										{formatFitnessSetTypeLabel(set.setType ?? "working")}
									</span>
									<Button
										variant="secondary"
										size="sm"
				...
				<p className="text-sm text-fitness-warm/65">
					{sk.fitness.setGestures.emptyCompletedSets}
				</p>
			)}
		</Card>
	);
...
function formatCompletedSetSummary(
...
function QuickAddExerciseCard({
	...
	const quickExerciseOptions = [...exerciseOptions]
		.sort(
			(left, right) =>
				getQuickExerciseOrder(left.name) - getQuickExerciseOrder(right.name) ||
	...
	return (
		<Card
			title="Rýchly štart bez plánu"
			description="Pridaj prvý cvik jedným ťuknutím. Pokročilý výber otvor iba vtedy, keď potrebuješ iný cvik alebo počet sérií."
		>
			{exerciseOptions.length > 0 ? (
						...
						<div className="mt-4 space-y-3">
							...
							<label className="block text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
								...
								<input
									...
									value={targetSets}
									onInput={(event) =>
										onTargetSetsChange(event.currentTarget.value)
									}
								/>
							</label>
							<Button
...
function FinishCheckInPanel({
	...
	return (
		<Card
			title="Krátka kontrola pred uložením"
			description="Voliteľné: pridaj RPE, energiu a poznámku iba vtedy, keď ti to pomôže pri ďalšom tréningu."
		>
			<div className="space-y-4">
				<div className="grid gap-3 sm:grid-cols-2">
					...
					<label className="block text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
						...
						<input
							...
							value={energyLevel}
							onInput={(event) =>
								onEnergyLevelChange(event.currentTarget.value)
							}
						/>
					</label>
				</div>
