...
export function SetLogger({
	...
	const fineWeightStep = displayUnit === "lb" ? 5 : 2.5;
	const largeWeightStep = displayUnit === "lb" ? 10 : 5;
	const maxDisplayWeight = convertWeightFromKg(500, displayUnit);
	const initialWeightEntryMode = set.weightEntryMode ?? "total";
	const initialLeftWeightKg =
		set.leftWeightKg ??
		(initialWeightEntryMode === "per_side" ? set.weightKg / 2 : 0);
	const initialRightWeightKg =
		set.rightWeightKg ??
		(initialWeightEntryMode === "per_side" ? set.weightKg / 2 : 0);
	const [weightEntryMode, setWeightEntryMode] =
		useState<FitnessWeightEntryMode>(initialWeightEntryMode);
	const [weight, setWeight] = useState(
		String(convertWeightFromKg(set.weightKg, displayUnit)),
	);
	const [leftWeight, setLeftWeight] = useState(
		String(convertWeightFromKg(initialLeftWeightKg, displayUnit)),
	);
	const [rightWeight, setRightWeight] = useState(
		String(convertWeightFromKg(initialRightWeightKg, displayUnit)),
	);
	const [barWeight, setBarWeight] = useState(
		String(getDefaultBarWeight(displayUnit)),
	);
	...
	const handleSubmit = async () => {
		...
		const leftWeightKg =
			weightEntryMode === "per_side"
				? convertWeightToKg(parsedLeftWeight, displayUnit)
				: null;
		const rightWeightKg =
			weightEntryMode === "per_side"
				? convertWeightToKg(parsedRightWeight, displayUnit)
				: null;
	...
	return (
		...
		>
			...
			{showRestCue ? (
				<div className="mt-4 rounded-2xl border border-fitness-yellow/25 bg-fitness-yellow/10 px-4 py-3">
					...
					<p className="mt-1 text-xs text-fitness-warm/65">
						Ťukaj rýchlo, drž techniku a potom zapíš sériu.
					</p>
				</div>
			) : null}
			...
			{weightEntryMode === "per_side" ? (
				<>
					...
					<p className="mt-2 rounded-2xl border border-fitness-yellow/20 bg-black/70 px-4 py-2 text-center text-xs font-black uppercase tracking-[0.14em] text-fitness-yellow">
						Spolu:{" "}
						{Number.isFinite(parsedPerSideTotalWeight)
							? formatDisplayNumber(parsedPerSideTotalWeight)
							: "—"}{" "}
						{displayUnit}
					</p>
			...
			<details
				...
				className="mt-4 rounded-2xl border border-fitness-yellow/25 bg-black/70 p-4"
			>
				<summary
					data-testid="plate-calculator-summary"
					className="flex cursor-pointer list-none items-start gap-3 [&::-webkit-details-marker]:hidden"
				>
					...
					<div className="min-w-0 flex-1">
						<p className="text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow">
							{sk.fitness.setLogger.plateCalculatorTitle}
						</p>
						<p className="mt-1 text-xs text-fitness-warm/65">
			...
			<div className="mt-4 grid grid-cols-3 gap-3">
				{weightEntryMode === "per_side" ? (
					<WeightQuickAdjustGroup
						label="Strany"
						targetLabel="obe strany"
						unit={displayUnit}
						fineStep={fineWeightStep}
						largeStep={largeWeightStep}
						onAdjust={(delta) => {
							setLeftWeight((current) =>
								adjustDecimal(current, delta, 0, maxDisplayWeight),
							);
							setRightWeight((current) =>
								adjustDecimal(current, delta, 0, maxDisplayWeight),
							);
						}}
						disabled={disabled}
...
function WeightEntryModeSelector({
	...
	return (
		<div className="mt-4 rounded-2xl border border-fitness-yellow/25 bg-black/70 p-4">
			...
			{value === "per_side" ? (
				<p className="mt-3 text-xs font-semibold text-fitness-warm/70">
					Pre jednoručky, rehab a asymetrické série. Do progresu ide súčet oboch
					strán.
				</p>
			) : null}
		</div>
		...
		<div className="rounded-2xl border border-fitness-yellow/20 bg-fitness-surface p-3 text-center">
			...
			>
				{[0, 1, 2, 3, 4].map((rirValue) => {
					const selected = selectedRir === rirValue;
					return (
						<button
							key={rirValue}
							type="button"
...
function SetTypeSelector({
	...
	return (
		<div className="mt-4 rounded-2xl border border-fitness-yellow/25 bg-black/70 p-4">
			...
			{value === "warmup" ? (
				<p className="mt-3 text-xs font-semibold text-fitness-warm/70">
					Rozcvička sa nepočíta do objemu, PR ani progresných odporúčaní.
				</p>
			) : null}
		</div>
	);
...
function WeightQuickAdjustGroup({
	label,
	targetLabel,
	unit,
	fineStep,
	...
	const adjustments = [
		{
			delta: -largeStep,
			stepLabel: formatStepLabel(largeStep, unit),
			icon: Minus,
		},
		{
			delta: -fineStep,
			stepLabel: formatStepLabel(fineStep, unit),
			icon: Minus,
		},
		{ delta: fineStep, stepLabel: formatStepLabel(fineStep, unit), icon: Plus },
		{
			delta: largeStep,
			stepLabel: formatStepLabel(largeStep, unit),
			icon: Plus,
	...
	return (
		<div className="rounded-2xl border border-fitness-yellow/20 bg-black/70 p-2 text-center">
			<p className="mb-2 text-[10px] font-black uppercase tracking-[0.14em] text-fitness-warm/50">
				{label}
			</p>
			<div className="grid grid-cols-2 gap-2">
				{adjustments.map(({ delta, stepLabel, icon: Icon }) => {
					const isDecrease = delta < 0;
					const ariaLabel = `${isDecrease ? "Znížiť" : "Zvýšiť"} ${targetLabel} o ${stepLabel}`;
					const text = `${isDecrease ? "−" : "+"}${stepLabel}`;
					return (
						<button
							key={`${delta}-${stepLabel}`}
							type="button"
...
function formatLastPerformance(
	lastPerformance: FitnessLastPerformance,
	displayUnit: FitnessDisplayUnit,
) {
	const weight = formatDisplayNumber(
		convertWeightFromKg(lastPerformance.weightKg, displayUnit),
	);
	const rir =
		lastPerformance.rir === null ? "RIR —" : `RIR ${lastPerformance.rir}`;
	const relativeTime = formatRelativeCompletedAt(lastPerformance.completedAt);
	return `Naposledy: ${weight} ${displayUnit} × ${lastPerformance.reps} @ ${rir}${relativeTime ? `, ${relativeTime}` : ""}`;
}

function formatRelativeCompletedAt(completedAt: string | null) {
	if (!completedAt) {
		return "";
	}
	...
	const daysAgo = Math.max(
		0,
		Math.floor((Date.now() - completedTime) / (24 * 60 * 60 * 1000)),
	);
	if (daysAgo === 0) {
		return "dnes";
	}
	...
	return `pred ${daysAgo} dňami`;
}

function formatStepLabel(step: number, unit: FitnessDisplayUnit) {
	return `${formatDisplayNumber(step)} ${unit}`;
}
