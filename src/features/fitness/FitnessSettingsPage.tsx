...
export function FitnessSettingsPage() {
	...
	const [coachModeEnabled, setCoachModeEnabledState] = useState(false);
	...
	const loadSettings = useCallback(async () => {
		setError(null);
		const [loadedSettings, loadedCoachModeEnabled] = await Promise.all([
			fitnessRepository.getSettings(),
			getCoachModeEnabled(),
		]);
		setSettings(loadedSettings);
		setCoachModeEnabledState(loadedCoachModeEnabled);
	}, []);
				...
				if (!cancelled) {
					setError(
						cause instanceof Error
							? cause.message
							: "Nepodarilo sa načítať tréningové nastavenia.",
					);
				}
			} finally {
				if (!cancelled) {
		...
		try {
			...
			setSuccessMessage(
				choice.outcome === "accepted"
					? sk.fitness.pwa.installAccepted
					: sk.fitness.pwa.installDismissed,
			);
		} catch (cause) {
			setError(
				cause instanceof Error ? cause.message : sk.fitness.pwa.installError,
			);
		}
	};
	...
	const updateDisplayUnit = async (displayUnit: FitnessDisplayUnit) => {
		...
		setSuccessMessage(null);
		try {
			const updatedSettings = await fitnessRepository.updateSettings({
				displayUnit,
			});
			...
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa uložiť zobrazovanú jednotku.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const updateGuidanceVisibility = async (showGuidance: boolean) => {
		...
		try {
			...
			setSettings(updatedSettings);
			setSuccessMessage(
				showGuidance ? "Pomocné texty zobrazené" : "Pomocné texty skryté",
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa uložiť viditeľnosť pomocných textov.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const updateRestSound = async (restSoundEnabled: boolean) => {
		...
		try {
			...
			setSettings(updatedSettings);
			setSuccessMessage(
				restSoundEnabled ? "Zvuk pauzy zapnutý" : "Zvuk pauzy vypnutý",
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa uložiť zvuk pauzy.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const updateRestVibration = async (restVibrationEnabled: boolean) => {
		...
		try {
			...
			setSuccessMessage(
				restVibrationEnabled
					? "Vibrácie pauzy zapnuté"
					: "Vibrácie pauzy vypnuté",
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa uložiť vibrácie pauzy.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const updateCoachMode = async (enabled: boolean) => {
		setIsMutating(true);
		setError(null);
		setSuccessMessage(null);
		try {
			await setCoachModeEnabled(enabled);
			setCoachModeEnabledState(enabled);
			setSuccessMessage(
				enabled
					? sk.fitness.coachMode.enabledSuccess
					: sk.fitness.coachMode.disabledSuccess,
			);
		} catch (cause) {
			setError(
				cause instanceof Error ? cause.message : sk.fitness.coachMode.saveError,
			);
		} finally {
			setIsMutating(false);
		}
	...
	const exportFitnessJson = async () => {
		...
		try {
			...
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa exportovať tréningové dáta.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const resetStarterData = async () => {
		...
		try {
			...
			setSuccessMessage(
				`Štartovacie dáta obnovené: ${result.starterPlanCount} štartovacie plány a ${result.starterExerciseCount} štartovacie cviky pripravené.`,
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa obnoviť štartovacie dáta.",
			);
		} finally {
			setIsMutating(false);
	...
	const deleteAllFitnessData = async () => {
		...
		try {
			...
			setSuccessMessage(
				"Tréningové dáta sú vymazané. Štartovacie šablóny môžeš znova obnoviť alebo importovať.",
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa vymazať tréningové dáta.",
			);
		} finally {
			setIsMutating(false);
	...
	const previewStrongCsvImport = () => {
		...
		try {
			...
			setSuccessMessage(
				`Náhľad Strong CSV: ${formatWorkoutCount(preview.workoutCount)}, ${formatExerciseCount(preview.exerciseCount)}, ${formatSetCount(preview.setCount)}.`,
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa pripraviť náhľad Strong CSV.",
			);
		}
	};
	...
	const importStrongCsv = async () => {
		...
		try {
			...
			setSuccessMessage(
				`Strong CSV import hotový: ${formatWorkoutCount(result.workoutCount)}, ${formatExerciseCount(result.exerciseCount)} a ${formatSetCount(result.setCount)} pridané do histórie.`,
			);
		} catch (cause) {
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa importovať Strong CSV.",
			);
		} finally {
			setIsMutating(false);
		}
	...
	const previewFitnessImport = () => {
		...
		try {
			...
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa pripraviť náhľad importu tréningového JSON.",
			);
		}
	};
	...
	const restoreFitnessImport = async () => {
		...
		try {
			const parsed = JSON.parse(importText) as unknown;
			const result = await fitnessRepository.importFitnessData(parsed, {
				mode: "replace",
			});
			const updatedSettings = await fitnessRepository.getSettings();
			setSettings(updatedSettings);
			...
			setError(
				cause instanceof Error
					? cause.message
					: "Nepodarilo sa obnoviť import tréningového JSON.",
			);
		} finally {
			setIsMutating(false);
			setPendingConfirmation(null);
			...
			<div className="space-y-6">
				<section className="fitness-hero-panel p-6 lg:p-8">
					...
					<h1 className="mt-4 text-4xl font-black tracking-[-0.06em] text-white sm:text-5xl">
						Súkromné tréningové dáta v zariadení.
					</h1>
					<p className="mt-3 max-w-2xl text-sm leading-6 text-fitness-warm/75">
						Bez prihlasovania, cloudu a telemetrie. Tréningové nastavenia
						používajú existujúcu lokálnu SQLite cestu.
					</p>
				</section>
				...
				{isLoading ? (
					<StatusMessage
						tone="info"
						message="Načítavam lokálne tréningové nastavenia…"
					/>
				) : null}
				{successMessage ? (
				...
				<Card
					...
					description="Najprv si sprav lokálnu zálohu. Až potom rieš importy, reset alebo technické nastavenia."
				>
						...
						<div className="flex items-start gap-3 rounded-2xl border border-fitness-yellow/30 bg-black px-4 py-4 text-fitness-warm">
							...
							<div>
								...
								<p className="mt-1 text-sm leading-6 text-fitness-warm/70">
									Stiahni si čitateľný JSON súbor iba s tréningovými dátami.
									Export sa pripraví v prehliadači a nič sa nikam neposiela.
								</p>
							</div>
						</div>
						<Button
							className="fitness-action h-fit"
							leadingIcon={<DatabaseBackup className="size-4" />}
							onClick={() => void exportFitnessJson()}
				...
				<details className="rounded-3xl border border-fitness-yellow/20 bg-black/55 p-4 text-fitness-warm">
					<summary className="cursor-pointer text-sm font-black uppercase tracking-[0.16em] text-fitness-yellow">
						Nastavenia tréningu a aplikácie
					</summary>
					<p className="mt-2 text-sm text-fitness-warm/65">
						Otvor, keď chceš meniť kg/lb, pomocné texty, signál pauzy,
						štartovacie dáta alebo PWA inštaláciu.
					</p>
					<section className="mt-4 grid gap-6 lg:grid-cols-3">
						<Card
							title={sk.fitness.coachMode.settingsTitle}
							description={sk.fitness.coachMode.settingsDescription}
						>
							<div className="rounded-2xl bg-fitness-yellow px-4 py-4 text-center text-2xl font-black text-black">
								coach
							</div>
							<p className="mt-3 text-sm font-semibold text-text-secondary dark:text-text-secondary-dark">
								{sk.fitness.coachMode.settingsStatus(coachModeEnabled)}
							</p>
							<Button
								type="button"
								className={cn(
									"mt-4 w-full",
									coachModeEnabled ? "" : "fitness-action",
								)}
								variant={coachModeEnabled ? "secondary" : "primary"}
								leadingIcon={<Users className="size-4" />}
								onClick={() => void updateCoachMode(!coachModeEnabled)}
								disabled={isLoading || isMutating}
							>
								{coachModeEnabled
									? sk.fitness.coachMode.disableButton
									: sk.fitness.coachMode.enableButton}
							</Button>
							<p className="mt-3 text-xs text-text-secondary dark:text-text-secondary-dark">
								Coach obrazovky sa objavia až po zapnutí. Dáta zostávajú lokálne
								a zdieľanie je len ručným exportom.
							</p>
						</Card>

						<Card
							title="Jednotky"
							description="Uložené váhy zostávajú v kg; zobrazenie môžeš prepínať bez zmeny histórie."
						>
							<div className="rounded-2xl bg-fitness-yellow px-4 py-4 text-center text-2xl font-black text-black">
								kg / lb
							</div>
							<p className="mt-3 text-sm font-semibold text-text-secondary dark:text-text-secondary-dark">
								Aktuálna jednotka zobrazenia: {displayUnit}
							</p>
							...
							<p className="mt-3 text-xs text-text-secondary dark:text-text-secondary-dark">
								Výpočty zostávajú v kg kvôli presnosti a konzistentnému exportu.
							</p>
						</Card>

						<Card
							title="Viditeľnosť pomoci"
							description="Skry voliteľné vysvetľujúce panely, keď chceš tichšiu aplikáciu."
						>
							<div className="rounded-2xl bg-fitness-yellow px-4 py-4 text-center text-2xl font-black text-black">
								pomoc
							</div>
							<p className="mt-3 text-sm font-semibold text-text-secondary dark:text-text-secondary-dark">
								Pomocné texty: {showGuidance ? "zapnuté" : "vypnuté"}
							</p>
							<div className="mt-4 grid grid-cols-2 gap-3">
								<Button
									type="button"
							...
							<p className="mt-3 text-xs text-text-secondary dark:text-text-secondary-dark">
								Hlavné ovládanie, chránené štítky a potvrdenia zostávajú
								viditeľné.
							</p>
						</Card>
						...
						<Card
							title="Signál pauzy"
							description="Upozornenie po skončení oddychu počas živého tréningu."
						>
							<div className="rounded-2xl bg-fitness-yellow px-4 py-4 text-center text-2xl font-black text-black">
								pauza
							</div>
							<div className="mt-3 space-y-1 text-sm font-semibold text-text-secondary dark:text-text-secondary-dark">
								...
								<p>
									Vibrácie pauzy: {restVibrationEnabled ? "zapnuté" : "vypnuté"}
								</p>
							</div>
							<div className="mt-4 grid grid-cols-2 gap-3">
								<Button
								...
								<Button
									...
									leadingIcon={<Zap className="size-4" />}
									onClick={() =>
										void updateRestVibration(!restVibrationEnabled)
									}
									disabled={isLoading || isMutating}
								>
							...
							<p className="mt-3 text-xs text-text-secondary dark:text-text-secondary-dark">
								Zvuk sa pripraví po prvom ťuknutí v tréningu; vibrácie fungujú
								len na podporovaných zariadeniach.
							</p>
						</Card>

						<Card
							title="Štartovacie dáta"
							description="Bezpečná obnova vstavaných štartovacích plánov."
						>
							<div className="flex items-center gap-3 rounded-2xl border border-fitness-yellow/30 bg-black px-4 py-4 text-fitness-warm">
								...
								<span className="text-sm">
									Obnov Tlak/Ťah/Nohy, Vrch/Spodok a Celé telo 3× bez vymazania
									osobných dát.
								</span>
							</div>
							<Button
								...
								onClick={() => setPendingConfirmation("resetStarterData")}
								disabled={isLoading || isMutating}
							>
						...
						<Card
							title={sk.fitness.pwa.installTitle}
							description={sk.fitness.pwa.installDescription}
						>
							<div className="space-y-3 rounded-2xl border border-fitness-yellow/30 bg-black px-4 py-4 text-fitness-warm">
								<p className="text-sm font-black text-fitness-yellow">
									{sk.fitness.pwa.addToHome}
								</p>
								<p className="text-sm text-fitness-warm/70">
									{sk.fitness.pwa.offlineTraining}
								</p>
								<p className="text-xs font-semibold text-fitness-warm/55">
									{sk.fitness.pwa.privatePromise}
								</p>
							</div>
							<Button
								className="fitness-action mt-4 w-full"
								leadingIcon={<ShieldCheck className="size-4" />}
								onClick={() => void installStingFit()}
								disabled={isLoading || isMutating || isStandalone}
							>
								{isStandalone
									? sk.fitness.pwa.installedButton
									: sk.fitness.pwa.installButton}
							</Button>
				...
				<details className="rounded-3xl border border-fitness-yellow/20 bg-black/55 p-4 text-fitness-warm">
					<summary className="cursor-pointer text-sm font-black uppercase tracking-[0.16em] text-fitness-yellow">
						Import, obnova a nebezpečná zóna
					</summary>
					<p className="mt-2 text-sm text-fitness-warm/65">
						Otvor iba vtedy, keď importuješ zo Strong, obnovuješ JSON alebo
					...
					<div className="mt-4 space-y-6">
						...
						>
							<div className="space-y-4">
								...
								{strongCsvPreview ? (
									<div className="rounded-2xl border border-fitness-yellow/30 bg-black px-4 py-4 text-sm text-fitness-warm">
										<p className="font-black text-fitness-yellow">
											Strong CSV náhľad pripravený
										</p>
										<p className="mt-1 text-fitness-warm/70">
											{formatWorkoutCount(strongCsvPreview.workoutCount)} ·{" "}
											{formatExerciseCount(strongCsvPreview.exerciseCount)} ·{" "}
											{formatSetCount(strongCsvPreview.setCount)}
											{strongCsvPreview.skippedRowCount > 0
												? ` · preskočené riadky: ${strongCsvPreview.skippedRowCount}`
												: ""}
										</p>
									</div>
								) : null}
								<div className="flex flex-wrap gap-3">
									<Button
										variant="secondary"
										onClick={previewStrongCsvImport}
								...
								<p className="text-xs text-text-secondary dark:text-text-secondary-dark">
									Váhy v lb sa prepočítajú na kg, RPE sa uloží ako približné RIR
									a neznáme cviky sa vytvoria ako vlastné importované cviky.
								</p>
							</div>
						</Card>

						<Card
							title="Obnova z tréningového JSON"
							description="Vlož tréningový export StingFit, skontroluj náhľad a potom lokálne nahraď iba tréningové tabuľky."
						>
							<div className="space-y-4">
								<label className="block text-xs font-black uppercase tracking-[0.18em] text-fitness-yellow/70">
								...
								<div className="flex flex-wrap gap-3">
									<Button
										variant="secondary"
										onClick={previewFitnessImport}
										disabled={isLoading || isMutating || !importText.trim()}
									>
									...
									<Button
										className="fitness-action"
										onClick={() =>
											setPendingConfirmation("restoreFitnessImport")
										}
										disabled={isLoading || isMutating || !importText.trim()}
									>
						...
						<Card
							...
							description="Vymaž iba tréningové tabuľky StingFit na tomto zariadení."
						>
							<div className="flex items-start gap-3 rounded-2xl border border-rose-500/30 bg-rose-500/10 px-4 py-4 text-sm text-rose-100">
								...
								<span>
									{
										"Vymaže cviky, plány, tréningové záznamy, série a tréningové nastavenia — iba tréningové dáta StingFit uložené na tomto zariadení."
									}
								</span>
							</div>
							<Button
				...
				<Card
					title="Sľub súkromia"
					description="Produkt zostáva lokálny a súkromný už v návrhu."
				>
					<div className="flex items-center gap-3 rounded-2xl border border-fitness-yellow/30 bg-fitness-yellow/10 px-4 py-4 text-sm text-text-secondary dark:text-text-secondary-dark">
						...
						Žiadne účty, predplatné, analytika, telemetria ani paywally nie sú
						súčasťou smerovania produktu.
					</div>
				</Card>
			</div>
