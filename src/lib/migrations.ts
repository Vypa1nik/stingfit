...
const profileSchema = [
    ...
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    kind TEXT NOT NULL CHECK(kind IN ('solo','coach','client')),
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    archived_at TEXT DEFAULT NULL
  )`,
	`CREATE INDEX IF NOT EXISTS idx_profiles_kind_active ON profiles(kind, archived_at)`,
] as const;
...
function ensureFitnessSetMetadataSchema(database: SqlRunner) {
	...
	if (!setColumns.includes("set_type")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN set_type TEXT NOT NULL DEFAULT 'working' CHECK(set_type IN ('working','warmup','dropset','myo','failure'))`,
		);
	}

	if (!setColumns.includes("weight_entry_mode")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN weight_entry_mode TEXT NOT NULL DEFAULT 'total' CHECK(weight_entry_mode IN ('total','per_side'))`,
		);
	}

	if (!setColumns.includes("left_weight_kg")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN left_weight_kg REAL DEFAULT NULL`,
		);
	}

	if (!setColumns.includes("right_weight_kg")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN right_weight_kg REAL DEFAULT NULL`,
		);
	}

	if (!setColumns.includes("corrected_at")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN corrected_at TEXT DEFAULT NULL`,
		);
	}

	if (!setColumns.includes("correction_count")) {
		database.run(
			`ALTER TABLE fitness_sets ADD COLUMN correction_count INTEGER NOT NULL DEFAULT 0`,
		);
	}
}
...
function ensureFitnessExerciseMuscleGroupSchema(database: SqlRunner) {
	...
	if (!exerciseColumns.includes("muscle_group")) {
		database.run(
			`ALTER TABLE fitness_exercises ADD COLUMN muscle_group TEXT DEFAULT NULL`,
		);
	}
}

function ensureFitnessSupersetSchema(database: SqlRunner) {
	const planExerciseColumns = getTableColumns(
		database,
		"fitness_plan_exercises",
	);
	const sessionExerciseColumns = getTableColumns(
		database,
	...
	if (!planExerciseColumns.includes("superset_group")) {
		database.run(
			`ALTER TABLE fitness_plan_exercises ADD COLUMN superset_group TEXT DEFAULT NULL`,
		);
	}

	if (!sessionExerciseColumns.includes("superset_group")) {
		database.run(
			`ALTER TABLE fitness_session_exercises ADD COLUMN superset_group TEXT DEFAULT NULL`,
		);
	}
}

function ensureFitnessSessionExerciseSnapshotSchema(database: SqlRunner) {
	const exerciseColumns = getTableColumns(
		database,
		"fitness_session_exercises",
	);
	...
	if (!exerciseColumns.includes("category_snapshot")) {
		database.run(
			`ALTER TABLE fitness_session_exercises ADD COLUMN category_snapshot TEXT DEFAULT NULL`,
		);
	...
	if (!exerciseColumns.includes("muscle_group_snapshot")) {
		database.run(
			`ALTER TABLE fitness_session_exercises ADD COLUMN muscle_group_snapshot TEXT DEFAULT NULL`,
		);
		database.run(`UPDATE fitness_session_exercises
...
function ensureFitnessSessionReviewSchema(database: SqlRunner) {
	...
	if (!sessionColumns.includes("session_rpe")) {
		database.run(
			`ALTER TABLE fitness_sessions ADD COLUMN session_rpe INTEGER DEFAULT NULL`,
		);
	}

	if (!sessionColumns.includes("energy_level")) {
		database.run(
			`ALTER TABLE fitness_sessions ADD COLUMN energy_level INTEGER DEFAULT NULL`,
		);
	}
}

function ensureDefaultProfile(database: SqlRunner) {
	database.run(`INSERT OR IGNORE INTO profiles(id, name, kind, created_at, archived_at)
    VALUES ('solo', 'Moje tréningy', 'solo', datetime('now'), NULL)`);
	database.run(`INSERT INTO app_settings(key, value, updated_at)
    SELECT 'active_profile_id', 'solo', datetime('now')
    WHERE NOT EXISTS (SELECT 1 FROM app_settings WHERE key = 'active_profile_id')`);
	database.run(`UPDATE app_settings
    SET value = 'solo', updated_at = datetime('now')
    WHERE key = 'active_profile_id'
      AND NOT EXISTS (
        SELECT 1
        FROM profiles
        WHERE profiles.id = app_settings.value
