...
export type DatabaseBootMetricStatus = "ready" | "error";

export interface DatabaseBootMetric {
	status: DatabaseBootMetricStatus;
	startedAtMs: number;
	completedAtMs: number;
	durationMs: number;
	sqlJsDurationMs: number;
	storageDurationMs: number;
	migrationDurationMs: number;
	error?: string;
}

interface DatabaseDebugChannel {
...
let latestDatabaseBootMetric: DatabaseBootMetric | null = null;
...
function durationMs(startedAtMs: number, completedAtMs: number) {
	return Math.max(0, Number((completedAtMs - startedAtMs).toFixed(2)));
}

function getDebugChannel() {
	const debugTarget = globalThis as typeof globalThis & {
		__STINGFIT_DEBUG__?: DatabaseDebugChannel;
	};

	debugTarget.__STINGFIT_DEBUG__ ??= {};
	return debugTarget.__STINGFIT_DEBUG__;
}

function publishDatabaseBootMetric(metric: DatabaseBootMetric) {
	latestDatabaseBootMetric = metric;

	const debugChannel = getDebugChannel();
	debugChannel.databaseBoot = metric;
	debugChannel.databaseBoots = [...(debugChannel.databaseBoots ?? []), metric];
}

function toErrorMessage(error: unknown) {
	return error instanceof Error ? error.message : "Unknown database boot error";
}
...
async function getSqlModule(): Promise<SqlJsStatic> {
	...
	const loadedSqlModule = await runtime.default({
		locateFile: () => getWasmLocateFile(),
	});

	sqlModule = loadedSqlModule;
	return loadedSqlModule;
}
...
export async function initDatabase() {
	...
	initPromise = (async () => {
		...
		let sqlJsDurationMs = 0;
		let storageDurationMs = 0;
		let migrationDurationMs = 0;

		try {
			const sqlStartedAtMs = nowMs();
			const SQL = await getSqlModule();
			sqlJsDurationMs = durationMs(sqlStartedAtMs, nowMs());

			const storageStartedAtMs = nowMs();
			const binary = toUint8Array(
				await get<Uint8Array | ArrayBuffer>(DATABASE_STORAGE_KEY, sqliteStore),
			);
			storageDurationMs = durationMs(storageStartedAtMs, nowMs());

			const migrationStartedAtMs = nowMs();
			const db = binary ? new SQL.Database(binary) : new SQL.Database();
			db.run("PRAGMA foreign_keys = ON");
			await runMigrations(db);
			seedBaseSettings(db);
			migrationDurationMs = durationMs(migrationStartedAtMs, nowMs());

			database = db;
			await persistDatabase();

			const completedAtMs = nowMs();
			publishDatabaseBootMetric({
				status: "ready",
				startedAtMs,
				completedAtMs,
...
export function getDatabaseBootMetric() {
	return latestDatabaseBootMetric;
}
