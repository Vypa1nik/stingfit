...
2. **Manual paired-device Coach Mode smoke remains outstanding.** `tests/coach-handoff-flow.test.ts` now covers coach `.stfplan` export -> fresh trainee import -> workout log -> trainee `.stfrecap` export -> coach read-only import as one automated rehearsal. Real coach device -> trainee device -> coach preview timing has not been completed in this environment.
